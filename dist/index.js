...
async function createLead(lead) {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Lead could not be stored because the database is unavailable");
    return;
  }
...
// server/routers.ts
var leadInputSchema = z2.object({
  name: z2.string().trim().min(2).max(160),
  phone: z2.string().trim().regex(/^[0-9+()\-\s]{7,32}$/, "Enter a valid phone number"),
  configuration: z2.enum(["3 BHK", "4 BHK"]),
  budget: z2.enum(["\u20B91 Cr", "\u20B92 Cr", "\u20B93 Cr", "\u20B94 Cr+"]),
  source: z2.string().trim().max(80).optional()
});
