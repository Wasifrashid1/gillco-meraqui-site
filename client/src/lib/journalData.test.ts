...
describe('Gillco Meraqui journal data', () => {
  ...
  it('retains the supplied metadata and an H2-led outline for every article', () => {
    expect(journalArticles.every(article => article.metaTitle.length > 0 && article.metaDescription.length > 0 && article.primaryKeyword.length > 0 && article.h1.length > 0)).toBe(true);
